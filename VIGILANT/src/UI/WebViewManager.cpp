...
void WebViewManager::StartHTTPServer() {
    std::cout << "WebView2 initialized with WebMessage API" << std::endl;
}

void WebViewManager::StopHTTPServer() {
    // Cleanup
}
void WebViewManager::OnEnvironmentCreated(ICoreWebView2Environment* env) {
    OutputDebugStringW(L"[WebViewManager::OnEnvironmentCreated] Called on UI thread\n");
    
    if (!env) {
        OutputDebugStringW(L"[WebViewManager::OnEnvironmentCreated] ERROR: env pointer is NULL\n");
        return;
    }

    m_environment = env;
    OutputDebugStringW(L"[WebViewManager::OnEnvironmentCreated] Environment stored\n");
    ...
    HRESULT hr = env->CreateCoreWebView2Controller(
        ...
        Microsoft::WRL::Callback<ICoreWebView2CreateCoreWebView2ControllerCompletedHandler>(
            [this](HRESULT result, ICoreWebView2Controller* controller) -> HRESULT {
                OutputDebugStringW(L"[WebViewManager::ControllerCallback] Controller creation callback\n");
                
                if (SUCCEEDED(result) && controller) {
                    controller->AddRef();
                    PostMessageW(m_hWnd, WM_APP + 2, (WPARAM)controller, 0);
                    OutputDebugStringW(L"[WebViewManager::ControllerCallback] Controller posted to UI thread\n");
                } else {
                    OutputDebugStringW(L"[WebViewManager::ControllerCallback] Controller creation failed\n");
                }
                return S_OK;
            }).Get());

    if (FAILED(hr)) {
        OutputDebugStringW(L"[WebViewManager::OnEnvironmentCreated] ERROR: CreateController call failed\n");
    }
}

void WebViewManager::OnControllerCreated(ICoreWebView2Controller* controller) {
    OutputDebugStringW(L"[WebViewManager::OnControllerCreated] Called on UI thread\n");

    try {
        if (!controller) {
            OutputDebugStringW(L"[WebViewManager::OnControllerCreated] ERROR: controller pointer is NULL\n");
            return;
        }

        m_controller = controller;
        HRESULT hrGetWebView = m_controller->get_CoreWebView2(&m_webView);

        if (FAILED(hrGetWebView) || !m_webView) {
            OutputDebugStringW(L"[WebViewManager::OnControllerCreated] ERROR: Failed to get WebView pointer\n");
            return;
        }

        OutputDebugStringW(L"[WebViewManager::OnControllerCreated] WebView pointer obtained\n");
        ...
        std::wstring testPath = fileUrlPath + L"/test.html";
        std::wstring fullTestPath = directory + L"\\test.html";

        HANDLE hFile = CreateFileW(fullTestPath.c_str(), GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
        bool testFileExists = (hFile != INVALID_HANDLE_VALUE);
        if (testFileExists) CloseHandle(hFile);

        // Setup message handler BEFORE navigation, not in NavigationCompleted callback
        // This prevents potential threading issues with COM objects
        OutputDebugStringW(L"[WebViewManager::OnControllerCreated] Setting up message handler\n");
        SetupMessageHandler();
        OutputDebugStringW(L"[WebViewManager::OnControllerCreated] Message handler setup complete\n");

        if (testFileExists) {
            m_webView->Navigate(testPath.c_str());
        } else {
            std::wstring dashboardPath = fileUrlPath + L"/dashboard.html";
            m_webView->Navigate(dashboardPath.c_str());
        }

        OutputDebugStringW(L"[WebViewManager::OnControllerCreated] Initialization complete\n");
    }
    catch (const std::exception& e) {
        OutputDebugStringW(L"[WebViewManager::OnControllerCreated] std::exception caught\n");
    }
    catch (...) {
        OutputDebugStringW(L"[WebViewManager::OnControllerCreated] Unknown exception caught\n");
    }
}
